...
The analysis is written for 2 genotypes. Certain elements may need to be edited for more than 2 genotypes (not tested)
...
#Example traces; Comment out if IncludeExampleTraces=FALSE
ExampleTrace1_1<-"./Data/M1503139415/CDKL5_345_Wake_Tmin2136.txt" #Transmitter27
ExampleTrace1_2<-"./Data/M1503139415/CDKL5_345_SWS_Tmin1936.txt" #Transmitter27
ExampleTrace1_3<-"./Data/M1503139415/CDKL5_345_REM_Tmin2200.txt" #Transmitter27
ExampleTrace2_1<-"./Data/M1503139415/CDKL5_357_Wake_Tmin2320.txt" #Transmitter27
ExampleTrace2_2<-"./Data/M1503139415/CDKL5_357_SWS_Tmin2664.txt" #Transmitter27
ExampleTrace2_3<-"./Data/M1503139415/CDKL5_357_REM_Tmin2960.txt" #Transmitter27

#Example spectra; Comment out if IncludeExamplePSD=FALSE
ExamplePSD1_1<-"./Data/PSD/M1470125501_Tmin3472_Spec.txt"
ExamplePSD1_2<-"./Data/PSD/M1470132701_Tmin312_Spec.txt"
ExamplePSD1_3<-"./Data/PSD/M1470132701_Tmin1280_Spec.txt" 
ExamplePSD2_1<-"./Data/PSD/M1470125501_Tmin3472_Spec.txt" 
ExamplePSD2_2<-"./Data/PSD/M1470132701_Tmin312_Spec.txt"
ExamplePSD2_3<-"./Data/PSD/M1470132701_Tmin1280_Spec.txt" 

#If IncludeVideoScored=FALSE, Comment out VideoScoreFileList and VideoScoreAnimalIDList
VideoScoreFileList<-c("./Data/VideoScoring/CDKL5_345_190817_1.csv",
        ...
        )  #VideoScoreFileList lists all .csv files with videoscoring. Files contain variables "Date", "Time", "S_W", e.g. 19/08/2017,12:15:31,S. Each row specifies a behavioural state and needs only to include the transition to the next behavioural state. The last entry must be the behavioural state at the end of the observation period.


VideoScoreAnimalIDList<-c("CDKL5_345",
                "CDKL5_346",
                "CDKL5_353", 
                "CDKL5_357", 
                "CDKL5_358"
            )  #VideoScoreAnimalIDList specifies AnimalID for each file in VideoScoreFileList
...
#Set logical variables for chunks to include
EvalNormaliseByBroadband<-TRUE
EvalSpikes<-TRUE
IncludeExampleTraces<-TRUE
IncludeExamplePSD<-TRUE
...
```{r NormaliseByBroadband, echo=FALSE, eval=EvalNormaliseByBroadband}
...
```{r ExampleTraces1Chunk, eval=IncludeExampleTraces, echo=FALSE}


Trace1<-read.csv(ExampleTrace1_1, header = FALSE)
Trace1<-Trace1*400*10^(-6)  #Data scaling for A3028B transmitter



Trace2<-read.csv(ExampleTrace1_2, header = FALSE)
Trace2<-Trace2*400*10^(-6)



Trace3<-read.csv(ExampleTrace1_3, header = FALSE)
...
ggsave("Output/Figures/ExampleTraces1.pdf",width=12,height=10, units="cm", device="pdf")
```


```{r ExamplesSpectra1Chunk, eval=IncludeExamplePSD, echo=FALSE}
Height=4
Width=7
FontSize=10

Spec1<-read.table(ExamplePSD1_1, header = FALSE, check.names = FALSE, sep=" ")
Freq<-as.vector(Spec1[1])
names(Freq)<-"Freq"
Spec1<-Spec1[2]
Spec1<-Spec1*((400*10^(-6))^2/10^(-6)) # Convert to V^2* 10e-6/Hz

Spec2<-read.table(ExamplePSD1_2, header = FALSE, check.names = FALSE, sep=" ")
Spec2<-Spec2[2]
Spec2<-Spec2*((400*10^(-6))^2/10^(-6))

Spec3<-read.table(ExamplePSD1_3, header = FALSE, check.names = FALSE, sep=" ")
Spec3<-Spec3[2]
Spec3<-Spec3*((400*10^(-6))^2/10^(-6))

Spec<-data.frame(c(Spec1, Spec2, Spec3))
names(Spec)<-c("PSD1", "PSD2", "PSD3")

Spec$Freq<-Freq$Freq

Spec.melted<-melt(Spec, id="Freq") # Arrange dataframe by frequency and add variable to identify state 1/2/3 for facet plot


Plot<-ggplot(data = Spec.melted, aes(x = Freq, y = value))+
    geom_line()+
    facet_grid(variable~.)+
    scale_y_continuous(limits=c(0, 30))+
    scale_x_continuous(limits=c(0.1, 256), breaks=c(2, 20, 200))+
    coord_trans(x="log10")+
    theme_bw()+
    ...
    labs(x="Frequency (Hz)", 
        y=expression(paste("Power (","10"^{-6},"V"^2, "/Hz)")))

Plot

ggsave("Output/Figures/PSD1.pdf", device="pdf", height=Height, width=Width, units="cm")

```

### `r Genotypes[2]`

```{r ExampleTraces2Chunk, eval=IncludeExampleTraces, echo=FALSE}


Trace1<-read.csv(ExampleTrace2_1, header = FALSE)
Trace1<-Trace1*400*10^(-6)



Trace2<-read.csv(ExampleTrace2_2, header = FALSE)
Trace2<-Trace2*400*10^(-6)



Trace3<-read.csv(ExampleTrace2_3, header = FALSE)
Trace3<-Trace3*400*10^(-6)
...
ggsave("Output/Figures/ExampleTraces2.pdf",width=12,height=10, units="cm", device="pdf")
```

```{r ExamplesSpectra2Chunk, eval=IncludeExamplePSD, echo=FALSE}
Height=4
Width=7
FontSize=10

Spec1<-read.table(ExamplePSD2_1, header = FALSE, check.names = FALSE, sep=" ")
Freq<-as.vector(Spec1[1])
names(Freq)<-"Freq"
Spec1<-Spec1[2]
Spec1<-Spec1*((400*10^(-6))^2/10^(-6)) # Convert to V^2* 10e-6/Hz

Spec2<-read.table(ExamplePSD2_2, header = FALSE, check.names = FALSE, sep=" ")
Spec2<-Spec2[2]
Spec2<-Spec2*((400*10^(-6))^2/10^(-6))

Spec3<-read.table(ExamplePSD2_3, header = FALSE, check.names = FALSE, sep=" ")
Spec3<-Spec3[2]
Spec3<-Spec3*((400*10^(-6))^2/10^(-6))
...
ggsave("Output/Figures/PSD2.pdf", device="pdf", height=Height, width=Width, units="cm")
...
```{r, Grouped_Broadband1, eval=EvalNormaliseByBroadband, echo=FALSE}
...
```{r, Grouped_Broadband2, eval=EvalNormaliseByBroadband, echo=FALSE}
...
```{r DeltaPlot1, eval=EvalNormaliseByBroadband, echo=FALSE}
...
```{r DeltaPlot2, eval=EvalNormaliseByBroadband,  echo=FALSE}
...
```{r ThetaPlot1, eval=EvalNormaliseByBroadband,  echo=FALSE}
...
```{r ThetaPlot2, eval=EvalNormaliseByBroadband, echo=FALSE}
...
```{r Gamma1Plot1, eval=EvalNormaliseByBroadband, echo=FALSE}
...
```{r Gamma2Plot2,eval=EvalNormaliseByBroadband,  echo=FALSE}
...
```{r Create_ECDFSpikes_chunk, eval=EvalSpikes,  echo=FALSE}
...
Plotting the cumulative frequency distributions of intervals with spikes in bins of 1 unit

```{r ECDFSpikes_PlotChunk, eval=EvalSpikes, echo=FALSE} 
...
``` {r ECDFSpikes_stats_chunk, echo=FALSE, eval=EvalSpikes}
...
```{r ScatterPlotChunk, eval=EvalSpikes,echo=FALSE}
...
for(FileIndex in 1:length(VideoScoreFileList)){
   VideoScoringFilename<-VideoScoreFileList[[FileIndex]] 
   AnimalIDCurrent<-VideoScoreAnimalIDList[[FileIndex]]
